...
def computeNeighborhood_without_current(S, K, N = 'all', nb = 'all', outputType = 'list'):
    """
    ...
    allNeighbors = [[] for n in range(S.shape[0])]
    ...
    if K > 0:
        ...
        for k in range(1,K):
            ...
            for i in range(N):
                ...
                neighbors_node = neighbors[i].copy()
                for j in neighbors_node:
...
def computeSourceNodes(A, C):
    ...
    for c in range(C):
        ...
        prediction_nodes = prediction_nodes + [communityNodes[degreeSorted[-1]]]

    nodesPerCommunity = {}
    for c in range(C):
        communityNodes = np.nonzero(communityLabels == c)[0]
        nodesPerCommunity[c] = communityNodes
        sourceNodes = sourceNodes + list(communityNodes)
    
    return sourceNodes, nodesPerCommunity, prediction_nodes
...
def createGraph(graphType, N, graphOptions):
    ...
    if graphType == 'SBM':
        ...
        p = graphOptions['probEdge']  # Edge probability
        G = nx.erdos_renyi_graph(N,p)  # create the graph
        A = nx.to_numpy_matrix(G)   # get adjacency matrix
        W = np.squeeze(np.asarray(A))  # convert adjacency matrix to ndarray

        assert W.shape[0] == W.shape[1] == N

    # print for checking type and shape
    print(f"Tipul matricei W este {type(W)}")
    print(f"Dimensiunile sunt f{W.shape}")
